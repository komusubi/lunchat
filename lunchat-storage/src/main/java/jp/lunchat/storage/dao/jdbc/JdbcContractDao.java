...
public class JdbcContractDao implements ContractDao {
    ...
    private static final String SELECT_RECORD_GROUPID_AND_SHOPID = "select " + COLUMNS
                    + " from contracts where groupId = ? and shopId = ?";
    private static final String SELECT_RECORD_GROUPCODE_AND_SHOPID = "select " + COLUMNS
                    + " from contracts where groupId = (select id from groups where code = ?) and shopId = ?";
...
//	          set.addAll(template.query(SELECT_QUERY_SHOPID, contractRowMapper, shopId));
    ...
    public Contract findByGroupCodeAndShopId(String groupCode, String shopId) {
        ...
        try {
            contract = template
                            .queryForObject(SELECT_RECORD_GROUPCODE_AND_SHOPID, contractRowMapper, groupCode, shopId);
        } catch (EmptyResultDataAccessException e) {
